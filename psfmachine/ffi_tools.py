...
class lazy_loader(object):
    def __init__(self, fnames, extensions, name="data", time_offset=2454833):
        ...
        hdrs = [
            fitsio.read_header(self.fnames[idx], ext=self.extensions[idx])
...
class Cube(object):
    ...
    @staticmethod
    def from_fnames(
        filenames=["kplr2009114174833_ffi-cal.fits"],
        ...
        **kwargs,
        ...
        if not np.in1d(error.time, data.time).all():
            raise ValueError(
                "Not all times are identical between input filenames and input error "
                "filenames"
            )
    ...
    def get_gaia_sources(self):
        ...
        # ra, dec = ra[0], dec[0]
        # ra2, dec2 = self.wcs[0].all_pix2world(
        #     np.asarray(
        #         [
        #             [
        #                 self.center[0] + self.shape[2] / 2,
        #                 self.center[1] + self.shape[1] / 2,
        #             ]
        #         ]
        #     ),
        #     0,
        ...
        ra, dec = (
            self.wcs[0]
            .all_pix2world(np.vstack([self.X.ravel(), self.Y.ravel()]).T, 0)
            .T
        )
        ...
        lmask = (
            (locs[:, 0] >= -1)
            & (locs[:, 0] <= self.shape[1] + 1)
            & (locs[:, 1] >= -1)
            & (locs[:, 1] <= self.shape[2] + 1)
        )
        sources, cs, locs = (
            sources[lmask].reset_index(drop=True),
            cs[lmask],
            locs[lmask],
        )
    ...
    def get_masks(self):
        ...
        # We're only going to calculate the masks when the rotation changes,
        # to save some time.
        if self.dmodule is None:
            dmodule = np.append(
                0, np.where(np.append(0, np.diff(self.data.extensions)) != 0)[0]
            )
        ...
        for tdx, d in enumerate(tqdm(dmodule[1:])):
            ...
            locs = self.wcs[dmodule[tdx]].wcs_world2pix(
                np.atleast_2d((cs.ra.deg, cs.dec.deg)).T, 0
            )
            dx1, dy1 = (self.X1[:, None] - locs[:, 0] - self.column), (
                self.Y1[:, None] - locs[:, 1] - self.row
            )
            ...
            masks, dx, dy, gaia_flux = (
                sparse.vstack(masks, "csc"),
                sparse.vstack(dx, "csc"),
                sparse.vstack(dy, "csc"),
                sparse.vstack(gaia_flux, "csc"),
            )
